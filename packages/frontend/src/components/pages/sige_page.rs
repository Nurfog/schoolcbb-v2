...
pub fn SigePage() -> Element {
    ...
    let students = use_resource(|| async move { client::fetch_json("/api/reports/sige/students").await });
    let mut year = use_signal(|| 2026);
    let mut month = use_signal(|| 5);
    let attendance = use_resource(move || async move {
        client::fetch_json(&format!("/api/reports/sige/attendance/{}/{}", year(), month())).await
    });
